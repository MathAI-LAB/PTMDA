...
# -*- coding: utf-8 -*-
"""
Matching normalization and Spurious target, Office 31,step1,
"""
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
...
import sys
sys.stdout = Logger( 'WD2a_S1_log1.txt')
...
dir_model= './WD2a1'
Sour1='/data2/yonghui_/data1_/Office31/webcam/'
Sour2='/data2/yonghui_/data1_/Office31/dslr/'
Targ='/data2/yonghui_/data1_/Office31/amazon/'
BSZs1= 36
BSZtg= 36
...
DTsetS1 = datasets.ImageFolder(Sour1, tfs_tr1)
LoaderS1 =DataLoader(DTsetS1,BSZs1,shuffle=True,drop_last=True, num_workers=4)

DTsetS2 = datasets.ImageFolder(Sour2, tfs_tr1)
LoaderS2 =DataLoader(DTsetS2,BSZs1,shuffle=True,drop_last=True, num_workers=4)

DTsetTGtr = datasets.ImageFolder(Targ, tfs_tr1)
LoaderTGtr =DataLoader(DTsetTGtr,BSZtg,shuffle=True,drop_last=True, num_workers=4)

DTsetTGte = datasets.ImageFolder(Targ, tfs_te1)
LoaderTGte =DataLoader(DTsetTGte,20,shuffle=False,drop_last=False, num_workers=4)

####
base_network=ResNetFc2('ResNet50',use_bottleneck=True, bottleneck_dim=256,new_cls=True,class_num=31)
...
for i in range(iters):   
    ...
    try:
        ...
        inputs_source, labels_source = source1_iter.next()
    try:
        inputs_target, _ = target_iter.next()
    except Exception as err:
        target_iter = iter(LoaderTGtr)
        inputs_target, _ = target_iter.next()
    ...
    sfLoss1=softNNLoss2(features,labels)
    total_loss = 1.0 * transfer_loss + classifier_loss+calc_coeff2(i)*sfLoss1
    total_loss.backward()
    optimizer.step()
    ...
    if AccTe0> best_acc:           
        best_acc = AccTe0
        torch.save(base_network, os.path.join(dir_model,'WD2a3_Rs50_{:03d}.pth'.format(i)))
        if best_idx>0:
            os.remove(os.path.join(dir_model,'WD2a3_Rs50_{:03d}.pth'.format(best_idx)))
        best_idx = i
