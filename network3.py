...
def alexnet(pretrained=False, **kwargs):
    ...
    return model
import torchvision.models.alexnet as alexnet0
...
class AlexNetFc2(nn.Module):
  def __init__(self, use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=1000):
    ...
    model_alexnet = alexnet0(pretrained=True)
  ...
  def forward(self, x, source=1):
    x = self.features(x)
    x = x.view(x.size(0), -1)                
    x = self.classifier(x)
    x = self.bottleneck(x) # dim=256, 
    if source == 1:
        y = self.fc(x)
        return x,y
    else:
...
def discrepancyL( out1, out2):
    return torch.mean(torch.abs(F.softmax(out1, dim =1) - F.softmax(out2, dim = 1)))

def Fvat( inputs, radius,Feat):
    ...
    outputs_classifier1= Feat(inputs)
    outputs_classifier1=outputs_classifier1.view(-1,10)
	
    outputs_classifier2= Feat(inputs + eps)
    outputs_classifier2=outputs_classifier2.view(-1,10)
    loss_p = discrepancyL(outputs_classifier1,outputs_classifier2)
...
def get_loss_vat( inputs, inputs_vat,Feat):
    outputs_classifier1 = Feat(inputs)
    outputs_classifier1=outputs_classifier1.view(-1,10)
    outputs_classifier2 = Feat(inputs_vat)
    outputs_classifier2=outputs_classifier2.view(-1,10)

    vat_loss = discrepancyL(outputs_classifier1, outputs_classifier2)
...
class AlexNetFc3(nn.Module):
  ...
  def forward(self, x, source=1):
    ...
    if source == 1:
        y = self.fc(x)
        if self.training:
            source_vat1 = Fvat(x, 0.5,self.fc)
            Svat_loss1 = get_loss_vat(x, source_vat1,self.fc)
        else:
            Svat_loss1 =0
        return x,y,Svat_loss1
    else:
        if self.training:
            source_vat1 = Fvat(x, 0.5,self.fc2)
            Svat_loss1 = get_loss_vat(x, source_vat1,self.fc2)
        else:
            Svat_loss1 =0
        y = self.fc2(x)
        return x, y,Svat_loss1
...
resnet_dict = {"ResNet18":models.resnet18, "ResNet34":models.resnet34, "ResNet50":resnet_tn.resnet50, "ResNet101":models.resnet101, "ResNet152":models.resnet152}
...
class ResNetFc2(nn.Module):
  ...
  def get_parameters(self):
    if self.new_cls:
        ...
        parameter_list = [{"params":self.parameters(), "lr_mult":1, 'decay_mult':2}]
    return parameter_list
class ResNetFc3(nn.Module):
  def __init__(self, resnet_name, use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=1000):
    super(ResNetFc3, self).__init__()
    model_resnet = resnet_dict[resnet_name](pretrained=True)
    self.conv1 = model_resnet.conv1
    self.bn1 = model_resnet.bn1
    self.relu = model_resnet.relu
